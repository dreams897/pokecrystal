	tilecoll WALL, WALL, WALL, WALL ; 00
	tilecoll WALL, FLOOR, WALL, FLOOR ; 01
	tilecoll WATER, WALL, WATER, WALL ; 02
	tilecoll WALL, WALL, WALL, WALL ; 03
	tilecoll FLOOR, FLOOR, WARP_CARPET_DOWN, WARP_CARPET_DOWN ; 04
	tilecoll WATER, WATER, WATER, WATER ; 05
	tilecoll WALL, WALL, FLOOR, FLOOR ; 06
	tilecoll FLOOR, FLOOR, FLOOR, FLOOR ; 07
	tilecoll WALL, WALL, FLOOR, FLOOR ; 08
	tilecoll FLOOR, WATER, FLOOR, WATER ; 09
	tilecoll WALL, WALL, WATER, WATER ; 0a
	tilecoll FLOOR, FLOOR, FLOOR, FLOOR ; 0b
	tilecoll FLOOR, WATER, FLOOR, WATER ; 0c
	tilecoll WATER, FLOOR, WATER, FLOOR ; 0d
	tilecoll FLOOR, FLOOR, WATER, WATER ; 0e
	tilecoll FLOOR, WATER, FLOOR, WATER ; 0f
	tilecoll WATER, WATER, WATER, WATER ; 10
	tilecoll WATER, FLOOR, WATER, FLOOR ; 11
	tilecoll WATER, WATER, FLOOR, FLOOR ; 12
	tilecoll WATER, FLOOR, FLOOR, FLOOR ; 13
	tilecoll WATER, WATER, FLOOR, FLOOR ; 14
